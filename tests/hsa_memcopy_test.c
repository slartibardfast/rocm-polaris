...
int main() {
    ...
    // Test 2: System->VRAM->System round-trip
    ...
    int fail_idx = -1;
    for (int i = 0; i < 256; i++) {
        if (dst[i] != (unsigned char)(i ^ 0xAA)) { fail_idx = i; ok = 0; break; }
    }
    printf("  round-trip: %s\n", ok ? "PASS" : "MISMATCH");
    if (fail_idx >= 0) {
    ...
    // Test 3: Fill VRAM with hsa_amd_memory_fill, read back
    ...
    printf("  fill check: 0x%08x 0x%08x 0x%08x 0x%08x\n",
           u32[0], u32[1], u32[2], u32[3]);
    ok = 1;
    for (int i = 0; i < 64; i++) {
        if (u32[i] != 0xDEADBEEF) { ok = 0; break; }
    }

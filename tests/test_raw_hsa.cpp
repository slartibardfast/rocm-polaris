...
int main(int argc, char **argv) {
    ...
    int *d_result;
    s = hsa_amd_memory_pool_allocate(g_gpu_pool, sizeof(int), 0, (void**)&d_result);
    if (s != HSA_STATUS_SUCCESS) { fprintf(stderr, "gpu alloc failed: %d\n", s); return 1; }
    hsa_amd_agents_allow_access(1, &g_cpu, NULL, d_result);

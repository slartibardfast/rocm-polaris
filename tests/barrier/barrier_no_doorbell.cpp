// Test: CP stalls on barrier dep=1, we set dep=0 from CPU WITHOUT re-ringing.
// Does the CP re-evaluate on its own, or does it need a doorbell?
#include <hsa/hsa.h>
...
int main() {
    ...
    // Wait for CP to stall on the barrier (dep=1)
    printf("Barrier submitted. Waiting 500ms for CP to stall...\n");
    usleep(500000);

    // Decrement dep using atomic sub + clflush (like bounce buffer)
    printf("Decrementing dep via sub+clflush (NO doorbell re-ring)...\n");
    ...
    // Poll comp directly (not through hsa_signal_wait which uses bounce buffer)
    printf("Polling comp for 5 seconds (no doorbell, no hsa_signal_wait)...\n");
    for (int i = 0; i < 50; i++) {
        int64_t v = __atomic_load_n(&((amd_signal_t*)comp.handle)->value, __ATOMIC_ACQUIRE);
        if (v == 0) {
            printf("PASS: CP re-evaluated barrier after %dms WITHOUT doorbell\n", (i+1)*100);
            goto done;
        }
        usleep(100000);  // 100ms
    }
    printf("FAIL: comp still %ld after 5s — CP did NOT re-evaluate without doorbell\n",
           __atomic_load_n(&((amd_signal_t*)comp.handle)->value, __ATOMIC_ACQUIRE));

    // Now re-ring doorbell
    printf("Re-ringing doorbell...\n");
    hsa_signal_store_relaxed(q->doorbell_signal, idx);
    for (int i = 0; i < 30; i++) {
        int64_t v = __atomic_load_n(&((amd_signal_t*)comp.handle)->value, __ATOMIC_ACQUIRE);
        if (v == 0) {
            printf("PASS: CP resolved after doorbell re-ring (%dms)\n", (i+1)*100);
            goto done;
        }
        usleep(100000);
    }
    printf("FAIL even after doorbell re-ring\n");

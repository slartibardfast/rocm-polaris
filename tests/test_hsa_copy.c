// test_hsa_copy.c — Use hsa_amd_memory_async_copy (ROCR blit) directly
// This bypasses CLR but uses ROCR's BlitKernel for D2H copy.
// If corruption appears → bug is in ROCR's blit path
// If clean → bug is in CLR's wrapper around the blit
//
// Build: gcc -O2 -o test_hsa_copy test_hsa_copy.c \
...
static hsa_amd_memory_pool_t g_sys_pool = {0}, g_ka_pool = {0}, g_gpu_pool = {0};
...
static hsa_status_t find_gpu_pool(hsa_amd_memory_pool_t p, void *d) {
    ...
    if (f & HSA_AMD_MEMORY_POOL_GLOBAL_FLAG_COARSE_GRAINED) g_gpu_pool = p;
    return HSA_STATUS_SUCCESS;
}
static hsa_status_t find_cpu_pool(hsa_amd_memory_pool_t p, void *d) {
    (void)d; hsa_amd_segment_t seg;
    hsa_amd_memory_pool_get_info(p, HSA_AMD_MEMORY_POOL_INFO_SEGMENT, &seg);
    if (seg != HSA_AMD_SEGMENT_GLOBAL) return HSA_STATUS_SUCCESS;
    uint32_t f;
...
int main(int argc, char **argv) {
    ...
    hsa_amd_agent_iterate_memory_pools(g_gpu, find_gpu_pool, NULL);
    hsa_amd_agent_iterate_memory_pools(g_cpu, find_cpu_pool, NULL);

    // Load kernel
    FILE *f = fopen("record_val_gfx803.co", "rb");
    if (!f) { fprintf(stderr, "No .co\n"); return 1; }
    ...
    // Queue for compute dispatch
    hsa_queue_t *q;
    hsa_queue_create(g_gpu, 1024, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &q);

    // VRAM buffer (kernel writes here)
    int *d_vram;
    hsa_amd_memory_pool_allocate(g_gpu_pool, sizeof(int), 0, (void**)&d_vram);
    hsa_amd_agents_allow_access(1, &g_cpu, NULL, d_vram);

    // System memory buffer (D2H copy destination)
    int *d_host;
    hsa_amd_memory_pool_t pool = g_ka_pool.handle ? g_ka_pool : g_sys_pool;
    hsa_amd_memory_pool_allocate(pool, sizeof(int), 0, (void**)&d_host);
    hsa_amd_agents_allow_access(1, &g_gpu, NULL, d_host);

    // Kernarg
    void *ka;
    hsa_amd_memory_pool_allocate(pool, 16, 0, &ka);
    hsa_amd_agents_allow_access(1, &g_gpu, NULL, ka);

    hsa_signal_t sig, copy_sig;
    hsa_signal_create(1, 0, NULL, &sig);
    hsa_signal_create(1, 0, NULL, &copy_sig);

    printf("Compute dispatch + hsa_amd_memory_async_copy D2H, %d iters...\n", iters);
    int fails = 0;

    for (int i = 0; i < iters; i++) {
        int expected = i * 7 + 13;

        // Dispatch compute kernel to write to VRAM
        *(uint64_t*)ka = (uint64_t)(uintptr_t)d_vram;
        *(int*)((char*)ka + 8) = expected;
        ...
        p->kernel_object = kobj; p->kernarg_address = ka;
        p->private_segment_size = psiz; p->group_segment_size = gsiz;
        ...
        // D2H copy via hsa_amd_memory_async_copy (uses ROCR BlitKernel)
        hsa_signal_store_relaxed(copy_sig, 1);
        hsa_status_t s = hsa_amd_memory_async_copy(d_host, g_cpu, d_vram, g_gpu,
                                                    sizeof(int), 0, NULL, copy_sig);
        if (s != HSA_STATUS_SUCCESS) {
            fprintf(stderr, "async_copy failed: %d\n", s);
            break;
        }
        hsa_signal_wait_scacquire(copy_sig, HSA_SIGNAL_CONDITION_LT, 1, UINT64_MAX, HSA_WAIT_STATE_ACTIVE);

        // Check result
        if (*d_host != expected) {
            fails++;
            if (fails <= 5)
                printf("  iter %d: exp %d got %d (0x%x)\n", i, expected, *d_host, *d_host);
        }
    }

    printf("hsa_copy: %d / %d (%.3f%%)\n", fails, iters, 100.0*fails/iters);

    hsa_signal_destroy(sig); hsa_signal_destroy(copy_sig);
    hsa_amd_memory_pool_free(ka); hsa_amd_memory_pool_free(d_vram);
    hsa_amd_memory_pool_free(d_host);
    hsa_queue_destroy(q);
